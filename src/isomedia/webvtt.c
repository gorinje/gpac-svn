...
typedef struct _webvtt_parser {
    ...
    GF_List              *samples;
    /* List of overlapping GF_WebVTTCue */
    GF_List              *cues;
...
static GF_Err gf_isom_webvtt_cue_add_property(GF_WebVTTCue *cue, GF_WebVTTCuePropertyType type, char *text_data, u32 text_len)
{
    char **prop;
    u32 len;
...
void gf_webvtt_parser_del(GF_WebVTTParser *parser)
{
    if (parser) {
        ...
        while (gf_list_count(parser->cues)) {
            gf_webvtt_cue_del((GF_WebVTTCue *)gf_list_get(parser->cues, 0));
            gf_list_rem(parser->cues, 0);
        }
        gf_list_del(parser->cues);
        gf_free(parser);
    }
}
...
static GF_Err gf_webvtt_add_cue_to_samples(GF_WebVTTParser *parser, GF_List *samples, GF_WebVTTCue *cue)
{
    ...
    if (cue_start > sample_end) {
        ...
        GF_WebVTTSample *empty_sample = gf_webvtt_sample_new();
    ...
    while (do_parse) {
        ...
        case WEBVTT_PARSER_STATE_WAITING_CUE:
            if (sOK && len) {
                if (strstr(szLine, "-->")) {
                    ...
                    gf_isom_webvtt_cue_add_property(cue, WEBVTT_ID, szLine, strlen(szLine));
                    parser->state = WEBVTT_PARSER_STATE_WAITING_CUE_TIMESTAMP;
                    break;
                }
            } else {
        ...
        case WEBVTT_PARSER_STATE_WAITING_CUE_TIMESTAMP:
            if (sOK && len) {
                ...
                strcpy(szSettings, "");
                if (sscanf(szLine, "%u:%u:%u.%u --> %u:%u:%u.%u%s", &cue->start.hour, &cue->start.min, &cue->start.sec, &cue->start.ms, 
                                                                    &cue->end.hour,   &cue->end.min,   &cue->end.sec,   &cue->end.ms, 
                                                                    szSettings) < 7) {
                    if (sscanf(szLine, "%u:%u.%u --> %u:%u.%u%s", &cue->start.min, &cue->start.sec, &cue->start.ms, 
                                                              &cue->end.min,   &cue->end.sec,   &cue->end.ms, 
                                                              szSettings) < 6) {
                        e = GF_CORRUPTED_DATA;
                ...
                } 
                {
                    ...
                    if (strlen(settings)) {
                        gf_isom_webvtt_cue_add_property(cue, WEBVTT_SETTINGS, settings, strlen(settings));
                    }
                }
        ...
        case WEBVTT_PARSER_STATE_WAITING_CUE_PAYLOAD:
            if (sOK && len > 0) {
                gf_isom_webvtt_cue_add_property(cue, WEBVTT_PAYLOAD, szLine, len);
...
GF_Err gf_webvtt_dump_header(FILE *dump, GF_WebVTTSampleEntryBox *wvtt)
{
    fprintf(dump, "%s\n\n", wvtt->config->string);
    return GF_OK;
}
...
GF_Err gf_webvtt_dump_iso_sample(FILE *dump, u32 timescale, GF_ISOSample *iso_sample)
{
    ...
    while(gf_bs_available(bs))
    {
        ...
        if (box->type == GF_ISOM_BOX_TYPE_VTCU) {
            ...
            if (cuebox->id) fprintf(dump, "%s\n", cuebox->id->string);
            fprintf(dump, "%u --> NEXT", iso_sample->DTS);
            if (cuebox->settings) fprintf(dump, "%s", cuebox->settings->string);
            fprintf(dump, "\n");
            if (cuebox->payload) fprintf(dump, "%s\n", cuebox->payload->string);
            fprintf(dump, "\n");
        }
        gf_isom_box_del(box);
    }
...
GF_List *gf_webvtt_parse_iso_cues(GF_ISOSample *iso_sample, u64 start)
{
    ...
    while(gf_bs_available(bs))
    {
        ...
        if (box->type == GF_ISOM_BOX_TYPE_VTCU) {
            ...
            if (cuebox->id) {
                gf_isom_webvtt_cue_add_property(cue, WEBVTT_ID, cuebox->id->string, strlen(cuebox->id->string));
            }
            if (cuebox->settings) {
                gf_isom_webvtt_cue_add_property(cue, WEBVTT_SETTINGS, cuebox->settings->string, strlen(cuebox->settings->string));
            }
            if (cuebox->payload) {
                gf_isom_webvtt_cue_add_property(cue, WEBVTT_PAYLOAD, cuebox->payload->string, strlen(cuebox->payload->string));
            }
        }
        gf_isom_box_del(box);
    }
...
GF_Err gf_webvtt_parse_iso_sample(GF_WebVTTParser *parser, u32 timescale, GF_ISOSample *iso_sample)
{
    ...
    wsample->start = iso_sample->DTS * 1000 / timescale;

    cues = gf_webvtt_parse_iso_cues(iso_sample, wsample->start);
            ...
            if (prev_wsample) {
                while (gf_list_count(prev_wsample->cues)) {
                    ...
                    if (
                        ...
                        if (old_cue->end.hour == 0 && 
                            old_cue->end.min == 0 && 
                            old_cue->end.sec == 0 && 
                            old_cue->end.ms == 0) old_cue->end = cue->start;
                        /* transfer the cue */
                        if (!has_continuation_cue) {
                            /* the cue can be safely serialized while keeping the order */
                            gf_list_add(parser->cues, old_cue);
                        } else {
...
GF_Err gf_webvtt_dump_cues(FILE *dump, GF_List *cues)
{
    ...
    for (i = 0; i < gf_list_count(cues); i++) {
        ...
        if (cue->start.hour || cue->end.hour) {
            fprintf(dump, "%02u:%02u:%02u.%03u --> %02u:%02u:%02u.%03u%s\n", cue->start.hour, cue->start.min, cue->start.sec, cue->start.ms, 
                                                                             cue->end.hour,   cue->end.min,   cue->end.sec,   cue->end.ms,
                                                                             (cue->settings ? cue->settings : ""));
        } else {
            fprintf(dump, "%02u:%02u.%03u --> %02u:%02u.%03u%s\n", cue->start.min, cue->start.sec, cue->start.ms, 
                                                                   cue->end.min,   cue->end.sec,   cue->end.ms,
                                                                   (cue->settings ? cue->settings : ""));
        }
        if (cue->text) fprintf(dump, "%s\n", cue->text);
        fprintf(dump, "\n", cue->text);
    }
    return GF_OK;
}
...
GF_Err gf_webvtt_dump_iso_track(GF_MediaExporter *dumper, char *szName, u32 track)
{
    ...
    wvtt = gf_webvtt_isom_get_description(dumper->file, track, 1);
    if (!wvtt) goto exit;
    e = gf_webvtt_dump_header(out, wvtt);
    ...
    for (i=0; i<count; i++) {
        ...
        e = gf_webvtt_parse_iso_sample(parser, timescale, samp);
        if (e) goto exit;
    ...
    gf_webvtt_dump_cues(out, parser->cues);
